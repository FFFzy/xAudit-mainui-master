...
let cid = ""
let filename = ""
const args = location.search.split("&")
args.forEach((item) => {
    let kv = item.replace("?", "").split("=")
    if (kv[0] == "cid") {
        cid = kv[1]
    } else if (kv[0] == "filename") {
        filename = kv[1]
    }
})
...
let Node = function (parent, gateway, index) {
    ...
    let gatewayAndHash = gateway.replace(':hash', cid) + "?filename=" + filename;
...
Node.prototype.checked = function () {
    ...
    if (!this.status.up) {
        ...
        this.link.innerHTML = `<a title="${host}" href="${url}" target="_blank">${host}</a>`;
